...
@Slf4j
public class ProfessionalWeeklyScheduleServiceFixed {
        ...
        try {
            ...
            for (Course course : courses) {
                ...
                if (scheduled.isPresent()) {
                    ...
                    log.info("Successfully scheduled: {} on {} at {}", 
                            course.getName(), "weekday", "time");
                } else {
    ...
    private EventResponse createEventResponse(TimeSlotOption option, LocalDate startOfWeek) {
        ...
        event.setDescription("Auto-scheduled course: " + option.getCourse().getSubject());
        ...
        event.setType(com.reservation.model.enums.EventType.COURSE);
        event.setStatus(com.reservation.model.enums.EventStatus.SCHEDULED);
    ...
    private int compareCoursesPriority(Course a, Course b) {
        // Sort by duration hours (higher first), then by name
        int durationComparison = Integer.compare(b.getDurationHours(), a.getDurationHours());
